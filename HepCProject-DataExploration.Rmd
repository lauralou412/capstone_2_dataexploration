...
length(unique(hep_c_icd_dx$PATIENT_ID)) #3707 unique patient id's


#Create vector with id numbers of unique patients with a Hep C icd code
#####
unique_hep_c_ids <- as.data.frame(unique(hep_c_icd_dx$PATIENT_ID))
colnames(unique_hep_c_ids) <- "PATIENT_ID"
unique_hep_c_ids$PATIENT_ID<-str_pad(unique_hep_c_ids$PATIENT_ID, 10, pad = "0")
nrow(unique_hep_c_ids) #3707
write.csv(unique_hep_c_ids, "Z:/unique_hep_c_patient_ids.csv")
```
...
nrow(subset(num_of_hep_c_codes_per_patient, total_num_of_hep_c_codes == 1)) #1288 unique patients with 1 hep c diagnosis code
nrow(subset(num_of_hep_c_codes_per_patient, total_num_of_hep_c_codes >= 5)) #1492 unique patients with 5 or greater hep c diagnosis code
```

```{r}
#####Find first hep c diagnosis dates; need to figure out date shifting before we can run this code???  It needs work####

#find earilest hep C icd code using aggregate 
hep_c_patients_first_hep_c_encounter_date <- aggregate(enc_dt_ds~patient_id,  data = hep_c_icd_dx, min) ####might not be able to use this encounter date!!!!!?????

#rename patient id and encounter date column
names(hep_c_patients_first_hep_c_encounter_date)[names(hep_c_patients_first_hep_c_encounter_date) == "enc_dt_ds"] <- "first_encounter_date_with_Hep_C_icd_code"
names(hep_c_patients_first_hep_c_encounter_date)[names(hep_c_patients_first_hep_c_encounter_date) == "patient_id"] <- "PATIENT_ID"
...
#######have to go through this code again - Cindy K. #######
...
hep_c_patient_merge_1 <- merge(hep_c_patient_encounters, diagnosis_icd, by = "PATIENT_ID", all.x = TRUE, all.y = FALSE)
dim(hep_c_patient_merge_1)

##Merge encounter data with cpt dataset
hep_c_patient_merge_2 <- merge(hep_c_patient_merge_1, cpt, by = "PATIENT_ID", all.x = TRUE, all.y = FALSE)

##Merge encounter data with med at admission
hep_c_patient_merge_3 <- merge(hep_c_patient_merge_2, med_rec_adm, by = "PATIENT_ID", all.x = TRUE, all.y = FALSE)

##Merge encounter data with meds at discharge
hep_c_patient_merge_4 <- merge(hep_c_patient_merge_3, med_rec_dc, by = "PATIENT_ID", all.x = TRUE, all.y = FALSE)

##Merge encounter data with med during inpatient stay
hep_c_patient_merge_5 <- merge(hep_c_patient_merge_4, meds_inpt, by = "PATIENT_ID", all.x = TRUE, all.y = FALSE)

##Merge encounter data with med during outpatient visit
hep_c_patient_merge_6 <- merge(hep_c_patient_merge_5, meds_outpt, by = "PATIENT_ID", all.x = TRUE, all.y = FALSE)

##Merge encounter data with procedure icd codes
hep_c_patient_merge_7 <- merge(hep_c_patient_merge_6, procedure_icd, by = "PATIENT_ID", all.x = TRUE, all.y = FALSE)
...
hep_c_patient_encounters_all <- merge(hep_c_patient_encounters, hep_c_patients_first_encounter_date [, c("patient_id", "first_encounter_date", "first_encounter_id")], by = "PATIENT_ID", all.x = TRUE, all.y = FALSE)
...
outpatient_encounters <- encounters[(encountes$Enc_EIO %in% c("O", "I, O", "E, O", "E, I, O")),]
...
write.csv(encounter_type_and_ID_df, "Z:/Hepatitis C Project/Cindy's Folder/encounter_type_and_ID_NA_freq_df.csv")
```
...
names(encounters)
encounter_dates <- encounters [,c("ENCOUNTER_ID", "Enc_DT_DS", "Appt_DTTM_DS", "Hsp_Adm_DTTM_DS")]
encounter_dates <- encounter_dates[!is.na(encounter_dates$ENCOUNTER_ID),]
dim(encounter_dates)
...
encounter_icd_diagnosis_date_merge <- merge(encounter_dates, diagnosis_icd_dates, by = "ENCOUNTER_ID")
...
encounter_cpt_date_merge <- merge(encounter_dates, cpt_dates, by = "ENCOUNTER_ID")
...
#merge with encounter table
#Evalute dates
meds_outpt_dates <- meds_outpt [,c("ENCOUNTER_ID", "Enc_DT_DS", "Order_DT_DS")]

encounter_meds_outpt_date_merge <- merge(encounter_dates, meds_outpt_dates, by = "ENCOUNTER_ID")
...
encounter_meds_outpt_date_merge$order_date_difference <- difftime(encounter_meds_outpt_date_merge$enc_dt_ds, encounter_meds_outpt_date_merge$Order_DT_DS, units = "days")
...
#merge with encounter table
#Evalute dates
meds_inpt_dates <- meds_inpt [,c("ENCOUNTER_ID", "HSP_ADM_DTTM_DS", "Order_DT_DS")]

encounter_meds_inpt_date_merge <- merge(encounter_dates, meds_inpt_dates, by = "ENCOUNTER_ID")
...
encounter_procedure_icd_date_merge <- merge(encounter_dates, procedure_icd_dates, by.x = "ENCOUNTER_ID", by.y = "encounter_id")
...
#merge with encounter table
#Evalute dates
social_hx_dates <- social_hx [,c("ENCOUNTER_ID", "Enc_DT_DS")]
colnames(social_hx_dates)[which(names(social_hx_dates ) == "Enc_DT_DS")] <- "Social_Enc_DT_DS"

encounter_social_hx_date_merge <- merge(encounter_dates, social_hx_dates, by="ENCOUNTER_ID")
